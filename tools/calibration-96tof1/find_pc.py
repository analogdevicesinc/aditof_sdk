...
def get_target_pc(cam_handle, code_dict, pc, target_ir, max_pc, cfg, multiply_factor):
    ...
    while(switch_count != 3):
        ...
        if multiply_factor != 1: #avoid writing into registers coresponding to mid
            data_mul[:] = pc * multiply_factor
            device.write_AFE_reg(cam_handle, np.array(to_multiply_keys), data_mul)

        data[:] = pc
        device.write_AFE_reg(cam_handle, np.array(list(code_dict.keys())), data)

        for i in range(5):
            frame.dummy_read(cam_handle)
        for frame_ind in np.arange(0, cfg['frame_count']):
            ir_image = frame.get_ir_image_df(cam_handle, cfg['raw_frame_width'], cfg['raw_frame_height'])
            ir_crop[frame_ind] = frame.crop_center(ir_image, cfg['frame_width'], cfg['frame_height'], cfg['window_x'], cfg['window_y'])
...
def run_find_pc(config_json, **kwargs):
    ...
    sweep_config_dict = load_config_dict(pc_config_dict['firmware_config_json_path'])
    ...
    lf_path = os.path.join(sweep_config_dict['firmware_path'],sweep_config_dict['repeat_num_filename'])
    file = open(lf_path)
    text = file.read()
    ...
    pc, pc_dict = get_target_pc(cam_handle, pc_dict, start_pc, target_ir, max_pc, sweep_config_dict, multiply_factor)
    ...
    pc, pc_dict = get_target_pc(cam_handle, pc_dict, pc, target_ir, max_pc, sweep_config_dict, multiply_factor)
    ...
    #Create new lf file
    repeat_folder = 'r_lfs'
    files = glob.glob(os.path.join(sweep_config_dict['firmware_path'], repeat_folder, '9_*'))
    new_filename = '9_RepeatNumAddrList_'+ str(len(files)) + '.lf'
    os.rename(lf_path, os.path.join(sweep_config_dict['firmware_path'], repeat_folder, new_filename))
    ...
    for key in pc_dict.keys():
        file.writelines(tohex(key,16,'04x') + '  ' + tohex(pc_dict[key],16,'04x') + '\n')
    file.close()
