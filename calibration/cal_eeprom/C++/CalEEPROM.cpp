...
{
    typedef std::unordered_map<float, packet_struct>::iterator calibration_map_iterator;
    for (calibration_map_iterator iter = calibration_map.begin(); iter != calibration_map.end(); iter++)
    {
        ...
        for (sub_iterator sub_iter = sub_packet_map.packet.begin(); sub_iter != sub_packet_map.packet.end(); sub_iter++)
        {
            std::cout << "\tSub Key: " << sub_iter->first;
            std::cout << "\tSub Size: " << sub_iter->second.size;
            std::cout << "\tSub Value: ";
            for (std::list<float>::iterator itt = sub_iter->second.value.begin(); itt != sub_iter->second.value.end(); itt++)
                std::cout << *itt <<" ";
            std::cout<< std::endl;
        }
    }
...
void SaveCalMap(std::unordered_map<float, packet_struct> calibration_map, char* file_name)
{
    ...
    for (calibration_map_iterator iter = calibration_map.begin(); iter != calibration_map.end(); iter++)
    {
        fwrite(&iter->first, 4, 1, f); //write float primary key (packet type - 4 bytes)

        packet_struct sub_packet_map = iter->second; 

        fwrite(&sub_packet_map.size, 4, 1, f); //write size of packet ( size is written in float format)
...
void ReadCalMap(std::unordered_map<float, packet_struct> &calibration_map, char* file_name)
{
    ...
    float key;
    while (fread(&key, 4, 1, fr) == 1)//Read key and check condition for end of file
    {
        float value;
        //std::cout << "Key " << key;
        packet_struct sub_packet_map;
        fread(&sub_packet_map.size, 4, 1, fr); //Read Size of sub packet (header,camera Intrinsic ..)
        //std::cout << "Size " << sub_packet_map.size << std::endl;

        float *sub_packet_value;
        sub_packet_value = (float *)malloc(sub_packet_map.size);
        fread(sub_packet_value, sub_packet_map.size, 1, fr); //Read Entire sub packet 

        for (int i = 0; i < sub_packet_map.size / (sizeof(float));) //Parse all the sub-packets
        {
            float parameter_key;
            parameter_key = sub_packet_value[i++]; //Parse key of parameter from sub packet
            sub_packet_map.packet[parameter_key].size = sub_packet_value[i++]; //Parse size of parameter from sub packet

            int number_elements = sub_packet_map.packet[parameter_key].size / sizeof(float);
            std::list <float> elements;
            for (int j = 0; j < number_elements; j++)
            {
                sub_packet_map.packet[parameter_key].value.push_back(sub_packet_value[i++]); //Parse size of parameter from sub packet
                
            }
            
            /*std::cout << "\tSub Key " << parameter_key;
            std::cout << "\tSub Size " << sub_packet_map.packet[parameter_key].size;
            std::cout << "\tSub Value " << sub_packet_map.packet[parameter_key].value << std::endl;*/
        }
...
float GetMapSize(std::unordered_map<float, packet_struct> calibration_map)
{
    ...
    for (calibration_map_iterator iter = calibration_map.begin(); iter != calibration_map.end(); iter++)
    {
        total_size = total_size + iter->second.size; //Add size of all the sub packets
    }
    return total_size;
}
...
float GetPacketSize(std::unordered_map<float, param_struct> packet)
{
    typedef  std::unordered_map<float, param_struct>::iterator sub_iterator;
    float packet_size = 0;
    for (sub_iterator sub_iter = packet.begin(); sub_iter != packet.end(); sub_iter++)
    {
        packet_size = packet_size + sub_iter->second.size + 8; //Added 8 for size of key and size
    }
    return packet_size;
}
...
void InitCalMap(std::unordered_map<float, packet_struct> &calibration_map)
{
    ...
    Header[EEPROM_VERSION].size = sizeof(Header[EEPROM_VERSION].value.size() * 4);
    ...
    CameraIntrinsic[EEPROM_VERSION].size = (CameraIntrinsic[EEPROM_VERSION].value.size() * 4);
    CameraIntrinsic[CAL_SER_NUM].value = { 0 };
    CameraIntrinsic[CAL_SER_NUM].size = (CameraIntrinsic[CAL_SER_NUM].value.size() * 4);
    CameraIntrinsic[CAL_DATE].value = { 12042019 };
    CameraIntrinsic[CAL_DATE].size = (CameraIntrinsic[CAL_SER_NUM].value.size() * 4);
    CameraIntrinsic[INTRINSIC].value = { 0, 0, 0, 0, 0, 0, 0 ,0 ,0 };
    std::cout << CameraIntrinsic[INTRINSIC].value.size()<<std::endl;
    CameraIntrinsic[INTRINSIC].size = (CameraIntrinsic[INTRINSIC].value.size() * 4);
    
    calibration_map[HEADER].size = GetPacketSize(Header);
    calibration_map[HEADER].packet = Header;
    calibration_map[CAMERA_INTRINSIC].size = GetPacketSize(CameraIntrinsic);//Added 8 for size of key and size
    calibration_map[CAMERA_INTRINSIC].packet = CameraIntrinsic;
    calibration_map[HEADER].packet[TOTAL_SIZE].value = { GetMapSize(calibration_map) };
}
