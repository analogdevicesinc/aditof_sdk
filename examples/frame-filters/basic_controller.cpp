...
Basic_Controller::Basic_Controller() {
    ...
    if (m_cameras.size()) {
        ...
        if (SELECTED_MODE > 2) {
            LOG(ERROR) << "Modes can be only near = 0, medium = 1, far = 2!";
            return;
        } else {
...
void Basic_Controller::setMode(int modeSelected) {
    aditof::Status status = aditof::Status::OK;
    auto camera = m_cameras[static_cast<unsigned int>(m_cameraInUse)];
    std::string modes_to_set[3] = {"near", "medium", "far"};
    ...
    if (modeSelected > 2) {
        ...
        status = camera->setMode(modes_to_set[modeSelected]);
        if (status != aditof::Status::OK) {
            LOG(ERROR) << "Could not set camera mode!";
        }
    }
...
int Basic_Controller::getRange() const {
